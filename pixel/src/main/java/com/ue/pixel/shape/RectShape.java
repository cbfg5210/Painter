...
public class RectShape extends BaseShape{
    ...
    public boolean onDraw(PxerView pxerView,int startX,int startY,int endX,int endY) {
        ...
        for (int i = 0; i < rectWidth+1; i++) {
            ...
            layerToDraw.setPixel(mX,startY,ColorUtils.compositeColors(pxerView.getSelectedColor(), layerToDraw.getPixel(mX,startY)));
            layerToDraw.setPixel(mX,endY,ColorUtils.compositeColors(pxerView.getSelectedColor(), layerToDraw.getPixel(mX,endY)));
        }
        for (int i = 1; i < rectHeight; i++) {
            ...
            layerToDraw.setPixel(startX, mY,ColorUtils.compositeColors(pxerView.getSelectedColor(), layerToDraw.getPixel(startX,mY)));
            layerToDraw.setPixel(endX, mY,ColorUtils.compositeColors(pxerView.getSelectedColor(), layerToDraw.getPixel(endX,mY)));
        }
